...
function build() {
  ...
  if (!util.isDirectory(dir + 'runtime/alaska-admin-view')) {
    throw new Error(dir + 'runtime/alaska-admin-view is not exists!');
  }
  ...
  console.log('webpack --config webpack.admin.pro.js');
  execFileSync(modulesDir + '.bin/webpack', ['--config', 'webpack.admin.pro.js'], {
    stdio: 'inherit'
  });
}
